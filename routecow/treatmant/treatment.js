  ...
  const UID=req.params.UID;
    ...
    router.post("/noti/:UID/:Date", (req, res) => {
      ...
      router.get("/graph/:uid/:startDate/:endDate/:cattleID", (req, res) => {
        ...
        firebase.firebase().ref("treatment/" + UID).orderByChild('datediagnose').startAt(startDate).endAt(endDate).once("value", (snapshot) => {
          const list = [];
          ...
          if(Cattle==="emtyp"){
            snapshot.forEach((elem) => {
              list.push(elem.val().datediagnose)
        
            })
          }
          else{
            snapshot.forEach((elem) => {
              if(Cattle===elem.val().id){
                list.push(elem.val().datediagnose)
              }
